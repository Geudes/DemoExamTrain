...
const { Specialization, DoctorSpecialization } = require('../models');
...
router.get('/', optionalAuthMiddleware, async (req, res) => {
  try {
    ...
    if (req.user?.role === 'doctor') {
      const selected = await DoctorSpecialization.findAll({
...
router.post('/selected', authMiddleware, requireRole('doctor'), async (req, res) => {
  try {
    ...
    for (const id of ids) {
      ...
      const [record] = await DoctorSpecialization.findOrCreate({
        where: { doctorId: req.user.id, specializationId: specId },
        defaults: { doctorId: req.user.id, specializationId: specId },
      });
      created.push({
...
router.delete('/selected', authMiddleware, requireRole('doctor'), async (req, res) => {
  try {
    const { specializationIds, specializationId } = req.body;
    const ids = specializationIds || (specializationId != null ? [specializationId] : []);
