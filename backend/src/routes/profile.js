...
const { User, TrainerProfile, Booking, TrainerSpecialization, RefreshToken } = require('../models');
const { authMiddleware } = require('../middleware/auth');
const { userToResponse, validateClientLevels, parseClientLevels } = require('../utils/helpers');
...
router.put('/me', async (req, res) => {
  try {
    ...
    if (user.role === 'trainer') {
      const [profile] = await TrainerProfile.findOrCreate({
      ...
      if (clientLevels !== undefined) {
        const levels = parseClientLevels(clientLevels);
        if (!validateClientLevels(levels)) {
          return res.status(400).json({ error: 'Invalid client levels' });
        }
        profile.clientLevels = levels;
      }
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    await TrainerSpecialization.destroy({ where: { trainerId: userId } });
    await Booking.destroy({ where: { clientId: userId } });
    await Booking.destroy({ where: { trainerId: userId } });
    await TrainerProfile.destroy({ where: { userId } });
